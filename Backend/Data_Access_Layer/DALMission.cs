...
namespace Data_Access_Layer
{
    public class DALMission
    {
        ...
        public List<Missions> MissionList()
        {
            ...
            return _cIDbContext.Missions.Where(m => !m.IsDeleted).ToList();
        }
        ...
        public string MissionApplicationPost(MissionApplicationDTO missionApplication)
        {
            string result = "";
            try
            {
                // Check if the user has already applied for the same mission
                bool applicationExists = _cIDbContext.MissionApplication.Any(ma => ma.MissionId == missionApplication.MissionId
                                                                                    && ma.UserId == missionApplication.UserId
                                                                                    && !ma.IsDeleted);

                if (!applicationExists)
                {
                    var user = _cIDbContext.UserDetail.FirstOrDefault(u => u.UserId == missionApplication.UserId);
                    if (user == null)
                    {
                    ...
                    var maxId = _cIDbContext.MissionApplication.Max(a => a.Id);

                    if( maxId == null)
                    {
                        throw new Exception("maxId not found!");
                    }
                    // Create a new mission application
                    var newMissionApplication = new MissionApplication
                    {
                        Id = maxId + 1,
                        MissionId = missionApplication.MissionId,
                        UserId = missionApplication.UserId,
                        AppliedDate = missionApplication.AppliedDate,
                        //UserImage= missionApplication.UserId.ToString(),
                        //UserName= user.Name,
                    ...
                    _cIDbContext.MissionApplication.Add(newMissionApplication);
                    _cIDbContext.SaveChanges();

                    result = "Mission application submitted successfully.";
                }
                else
                {
                    throw new Exception("The user has already applied for this mission.");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (!missionExists)
                {
                    ...
                    if (missionToUpdate != null)
                    {
                       ...
                       // mi//ssionToUpdate.MissionAvilability = mission.MissionAvilability;
