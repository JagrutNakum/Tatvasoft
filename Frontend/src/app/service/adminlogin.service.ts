...
export class AdminloginService {
  ...
  registerUser(user:user){
      return this.http.post(`${this.apiUrl}/Login/Register`,user,{responseType:'json'});
  }

  GetUserById(id: number): Observable<user[]> {
    return this.http.get<user[]>(
      `${this.apiUrl}/Login/GetUserById/${id}`
    );
  }
  UpdateUser(data: user) {
    return this.http.post(`${this.apiUrl}/Login/UpdateUser`, data);
  }
