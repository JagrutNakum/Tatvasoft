  ...
  CountryList() {
    this.service.CountryList().subscribe((data: any) => {
      if (data.status == "Success") {
        ...
        this.toast.error({ detail: "ERROR", summary: "Error fetching countryList" /*data.message*/, duration: 3000 });
      }
    });
  }
  ...
  CityList(countryId: any) {
    this.service.CityList(countryId).subscribe((data: any) => {
      if (data.status == "Success") {
        ...
        this.toast.error({ detail: "ERROR", summary:"Error fetching CityList" /*data.message*/, duration: 3000 });
      }
    });
  }
