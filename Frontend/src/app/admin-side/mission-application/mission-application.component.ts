...
export class MissionApplicationComponent implements OnInit {
  ...
  ApproveMissionApplication(applicationId:any ) {
    console.log(applicationId);
    this.service.MissionApplicationApprove(applicationId).subscribe(
      (data: any) => {
        if (data.result == 1) {
  ...
  DeleteMissionApplication(applicationId: any) {
    this.service.MissionApplicationDelete(applicationId).subscribe(
      (data: any) => {
        if (data.result == 1) {
          this.toast.success({detail:"SUCCESS",summary:data.data,duration:3000});
          setTimeout(() => {
